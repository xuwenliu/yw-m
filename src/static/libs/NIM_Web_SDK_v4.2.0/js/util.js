...
function getAvatar(url) {
    ...
    if (re.test(url)) {
        return url + ((url.indexOf("?")>0)? "&imageView2/2/w/64":"?imageView2/2/w/64");
    } else {
        return "/static/libs/NIM_Web_SDK_v4.2.0/images/default-icon.png"
    }
}
...
function getTeamMembers(teamId,nims,caches){
    ...
    function getTeamMembersDone(error, obj) {
        console.log(error);
        console.log("获取群成员",obj);
        console.log('获取群成员' + (!error?'成功':'失败'));
        if (!error) {
            var teamId = obj.teamId;
            var members = obj.members;
            caches.teamMembers = caches.teamMembers || {};
            caches.teamMembers[teamId] = nim.mergeTeamMembers(caches.teamMembers[teamId], members);
            caches.teamMembers[teamId] = nim.cutTeamMembers(caches.teamMembers[teamId], members.invalid);
            //获取群名片
            var arr=caches.teamMembers[teamId];
            getNicks(nims,caches,arr);
...
function getNicks(nims,caches,teamMembers){
    var accounts=[],
        arr=teamMembers||[];
    for (var j in arr){
        if(arr[j]["account"]){
            accounts.push(arr[j]["account"]);
        }
    }
    nims.getUsers({
        accounts: accounts,
    ...
    function getUsersDone(error, users) {
        ...
        if (!error&&users) {
            caches.users=caches.users||[];
            caches.users = nims.mergeUsers(caches.users, users);
            var arr=caches.users||[];
            caches.usersObj=caches.usersObj||{};

            for(var k in arr ){
                var account=arr[k]["account"];
                if(account){
                    caches.usersObj[account]=arr[k];
                }
            }
            console.log("群成员名片列表",caches.usersObj);
...
// * **/
//
//function getTeamMembers(teamId,callB){
//    var nims=window.isMine===true?nim:nimTemp;
//    var caches=window.isMine===true?nimCache:nimCacheTemp;
//
//    nims.getTeamMembers({
//        teamId: teamId,
//        done: getTeamMembersDone
...
//            //获取群名片
//            var arr=caches.teamMembers[teamId];
//            getNicks(nims,caches,arr,callB);
//
//        }
//    }
//}
...
// * **/
//
//function getNicks(nims,caches,teamMembers,callB){
//    var accounts=[],
//        arr=teamMembers||[];
//    for (var j in arr){
//        if(arr[j]["account"]){
...
function getNick(account) {
    ...
    var nick="",members=$.extend(localStorage.localMembers&&JSON.parse(localStorage.localMembers),nimCache['usersObj'],nimCacheTemp['usersObj']);
    localStorage.localMembers=JSON.stringify(members);//存入缓存
    nick= members[account]&&members[account]["nick"]||account;
    return nick
}
