...
/*引入七牛*/
//console.log(parent.window.uptk)
var Qiniu = new QiniuJsSDK();
var uploader =function(t){
    Qiniu.uploader({
        runtimes: 'html5,flash,html4',      // 上传模式，依次退化
        browse_button: 'qiniuBtn',         // 上传选择的点选按钮，必需
        uptoken:parent.window.uptk,
        //uptoken_url: window.API.OTHER.QINIU_UPTOKEN,            //Ajax请求upToken的Url，**强烈建议设置**（服务端提供）
        //若未指定uptoken_url,则必须指定 uptoken ,uptoken由其他程序生成
        //uptoken_func:"",
        unique_names: true, // 默认 false，key为文件名。若开启该选项，SDK为自动生成上传成功后的key（文件名）。
        // save_key: true,   // 默认 false。若在服务端生成uptoken的上传策略中指定了 `sava_key`，则开启，SDK会忽略对key的处理
        domain:'http://o8nljewkg.bkt.clouddn.com/', //bucket 域名，下载资源时用到，**必需**
        ...
        init: {
            ...
            'FileUploaded': function(up, file, info) {
                ...
                var domain = up.getOption('domain'),
                    ...
                    sizeRes=$("body").attr("data-sizes"),
                    sourceLink = domain + res.key+sizeRes;

                $(".edui-image-searchTxt").val(sourceLink+"&imageView2/2/w/750");
                $("#remote input#url").val(sourceLink+"&imageView2/2/w/750");
                t.setPreview();//自动执行展示
